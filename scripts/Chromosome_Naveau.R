...
# args=c("sharp","0.10","TRUE","c('covariate','correlation')")
buildMethod = args[1]
...
if(!file.exists(paste0("data/applicationFiles/Plant/chromosome/data_file",ch,".txt"))){
  ...
  V_tilde_ch= as.matrix(cbind(sspop,V_ch))
  
  V_tilde_ch= as.matrix(V_tilde_ch)
  
  V_tilde_ch <- scale(V_tilde_ch,center=T,scale=T)
  ...
  V_tilde=cbind(id=1:n,data_ch$V_tilde)
  
  data = merge(cbind(id=Id,time=t,obs=Y),V_tilde,by="id")
  colnames(data)[which(stringr::str_detect(colnames(data),"\\."))] <- stringr::str_replace_all(colnames(data)[which(stringr::str_detect(colnames(data),"\\."))],"\\.","_")
  Sys.sleep(0.1)
  write.csv(data,file=paste0("data/applicationFiles/Plant/chromosome/data_file",ch,".txt"),quote = F,row.names = F)
}

sim = read.csv(paste0("data/applicationFiles/Plant/chromosome/data_file",ch,".txt"))
nbCov = ncol(sim) - 3
covToTest = colnames(sim[,-c(1:8)])

newProject(modelFile = "data/modelFiles/NaveauApp.txt",
...
setIndividualParameterDistribution(psi="normal")
setErrorModel(obs="constant")
setCovariateModel(phi=setNames(rep(TRUE,5),paste0("V",1:5)))
