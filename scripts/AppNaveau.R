...
data <- read.csv2("data/applicationFiles/plant/13INRmon_NUE_LN.csv")
head(data)
...
data_obs=newdata1
save(data_obs,file="data/applicationFiles/plant/data_obs.Rdata")

###################### Data-set with the SNPs ######################

SNPs <- read.csv2("data/applicationFiles/plant/genotypeFilterPlink_100_5_0.8.csv",sep="",row.names = 1)
SNPs <- as.matrix(SNPs)
SNPs <- t(SNPs)
...
data_chromosome <- read.csv2("data/applicationFiles/plant/carte_Axiom-TABW420k_WGAv1.csv",header=TRUE, sep = " ") #dataframe containing the position on the genome of each SNP
...
heading_QTL <- read.csv("data/applicationFiles/plant/marker_HD_INRmon13LN.csv",sep="")[,1]  #list of heading QTLs names
heading_QTL <- as.character(heading_QTL)

save(data_chromosome,SNPs,heading_QTL,file="data/applicationFiles/plant/data_ch.RData")
...
V_tilde=cbind(id=1:n,data_chr6A$V_tilde[,-1])

data = merge(cbind(id=Id,time=t,obs=Y),V_tilde,by="id")
write.csv(data,file=paste0("data/applicationFiles/plant/chromosome/",ch,".txt"),quote = F,row.names = F)
...
sspop <- readRDS("data/applicationFiles/plant/resPCOAdf.Rds")
data_chromosome <- read.csv2("data/applicationFiles/plant/carte_Axiom-TABW420k_WGAv1.csv",header=TRUE, sep = " ")
...
source('scripts/Functions_SAEMVS_real_data.R')
