sharp.SerialRegression <- function (xdata,
                                    ydata = NULL,
                                    ymeans, 
                                    Lambda,
                                    list_rep = NULL,
                                    ...
                                    verbose = TRUE, ...) 
{
  ...
  if (length(dim(mybeta$beta_full)) == 2) {
    Beta_full <- array(0, dim = c(dim(mybeta$beta_full)[1], 
                                  dim(mybeta$beta_full)[2], K), dimnames = list(rownames(mybeta$beta_full), 
                                                                                dimnames(mybeta$beta_full)[[2]], NULL))
  }
  else {
    if (length(dim(mybeta$beta_full)) == 3) {
      Beta_full <- array(0, dim = c(dim(mybeta$beta_full)[1], 
                                    dim(mybeta$beta_full)[2], K, dim(mybeta$beta_full)[3]), 
  ...
  for (k in 1:K){
    ...
    Xsub <- xdata
    Ysub <- ydata[[list_rep[k]]]
    mybeta <- sharp:::SelectionAlgo(xdata = Xsub, ydata = Ysub, 
                                    Lambda = Lambda[, 1], group_x = group_x, family = family, 
                                    implementation = implementation, ...)
    
    Beta[rownames(mybeta$selected), colnames(mybeta$selected), 
         k] <- mybeta$selected
    if (length(dim(Beta_full)) == 3) {
      Beta_full[rownames(mybeta$beta_full), colnames(mybeta$beta_full), 
                k] <- mybeta$beta_full
    }
    else {
      Beta_full[rownames(mybeta$beta_full), colnames(mybeta$beta_full), 
  ...
  if (group_penalisation) {
    ...
    metrics <- sharp:::StabilityMetrics(selprop = bigstab, pk = NULL, 
                                pi_list = pi_list, K = K, n_cat = n_cat, Sequential_template = NULL, 
  ...
  out <- list(S = metrics$S, Lambda = Lambda, Q = metrics$Q, 
              ...
              methods = list(type = "variable_selection", implementation = myimplementation, 
                             family = family, resampling = resampling, cpss = cpss, 
                             PFER_method = PFER_method), params = list(K = K, 
