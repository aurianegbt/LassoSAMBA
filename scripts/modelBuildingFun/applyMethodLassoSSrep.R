applyMethodLassoSSrep <- function(Y,X,omega,cov0,
  ...
  if(!is.null(exclude) && ncol(Xwh)-length(exclude)==0){
    ...
    VariableSelection.outputs = sharp.VariableSelection(Xwh,Ywh,Ywh.means,exclude=exclude,nfolds=nfolds,pi_list=seq(0.50,0.99,0.01),alpha=alpha,K=max(Ywh[,1]),n_cores=n_cores,FDP_thr = FDR_thr)
