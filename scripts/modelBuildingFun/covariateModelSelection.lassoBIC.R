covariateModelSelection.lassoBIC <- function(alpha = 1,
                                          covFix = NULL,
                                          pen.coef=NULL,
                                          weight=1,
                                          paramToUse="all",
  ...
  r.var = foreach(p = names(indvar)[which(indvar)],.export = c("applyMethodLassoBIC","modelFromSelection"),.packages = c("glmnet")) %dopar% {
    aux=applyMethodLassoBIC(Y.mat[,stringr::str_detect(colnames(Y.mat),p),drop=F],
                         X.mat,
                         Sigma[p,p],
                         cov0=cov0.list[[p]],
                         p.name=p,
