covariateModelSelection.lassoSSrep <- function(nfolds = 5,
                                               alpha = 1,
                                               covFix = NULL,
                                               pen.coef=NULL,
                                               weight=1,
  ...
  Y.means = sapply(Y[,-c(1,2),drop=F],function(x){rowMeans(matrix(x,nrow=N))})
  
  r.var = foreach(p = names(indvar)[which(indvar)],.export = c("applyMethodLassoSSrep","modelFromSelection","sharp.VariableSelection","sharp.SerialRegression","PenalisedRegression")) %dopar% {
    aux=applyMethodLassoSSrep(Y.mat[,c(1,which(stringr::str_detect(colnames(Y.mat),p))),drop=F],
                         X.mat,
                         Sigma[p,p],
                         cov0=cov0.list[[p]],
                         Y.means = Y.means[,stringr::str_detect(colnames(Y.means),p),drop=F],
                         p.name=p,
                         nfolds=nfolds,
                         alpha=alpha,
                         criterion=criterion,
