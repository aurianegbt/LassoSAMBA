...
for(i in 1:100){
  ...
  covTable = merge(sim$IndividualParameters$simulationGroup1[sim$IndividualParameters$simulationGroup1$rep==i,c("id","AGE","G1","G2","G3")],genCov(distribution,individualsNumber = 100,covariateChar = "Gen"),by="id") %>% 
...
annotate_figure(corrplot)
ggsave("Simu/corrPasinMoreZoom.png",
       height = 6000, width = 6000, units = "px", bg='transparent')
...
def <- defData(def,varname="G3",formula=0,variance=1,dist="normal")
for(i in 1:496){
...
for(i in 1:100){
  ...
  write.csv(covTable [,1:5],"tmpfile.txt",quote = F,row.names = F)
