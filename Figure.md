...
```
The following graphs is the colored figure generated in black and white by the code below.

<p align="center">
<img src="outputs/figures/finalFigures/Figure2_colored.jpeg" alt="Observed Data" width="800"/>
</p>

<p align="center"><strong>Figure 1:</strong> Proportion of exact models (no false positives or false negatives) and models that strictly include the exact model (no false negatives but with false positives). (A) Pharmacokinetic model with categorical covariates ($N=200$, $p=500$); (B) Vaccinology framework with Gaussian-correlated covariates ($N=100$, $p=1000$); (C) Vaccinology framework with randomly drawn correlated covariates ($N=100$, $p=200$).</p>
...
```r
ggarrange(bw(pN$ParComp+theme(plot.background = element_rect(linewidth=0.7,color="black"))),
...
<p align="center">
<img src="outputs/figures/finalFigures/Figure3_colored.jpeg" alt="Observed Data" width="800"/>
</p>

<p align="center"><strong>Figure 2:</strong> Covariate selection frequency across simulation frameworks for the step-SAMBA method, the SAEMVS method and the Lasso-SAMBA method, with an error control threshold of 10\%. The mean selection frequency of false discoveries is shown on the right side of each histogram.(A) Pharmacokinetic model with categorical covariates ($N=200$, $p=500$); (B) Vaccinology framework with Gaussian-correlated covariates ($N=100$, $p=1000$); (C) Vaccinology framework with randomly drawn correlated covariates ($N=100$, $p=200$).</p>
...
<p align="center">
<img src="outputs/figures/finalFigures/Figure4.jpeg" alt="Observed Data" width="800"/>
</p>
