modelFromLassoSelection <- function(Y,X,selection=NULL,Sigma=NULL,alpha=1,cov0.list=NULL,stabilitySelection=TRUE,nfolds=5,nSS=1000,thresholdsSS=0.90,parallel=FALSE,ncores=1){
  if(!is.data.frame(Y)){
    Y <- as.data.frame(Y)
  }
  if(!is.data.frame(X)){
