buildmlx <- function(project=NULL, final.project=NULL, model="all", prior=NULL, weight=NULL, coef.w1=0.5,
                     ...
                     stabilitySelection=TRUE,nSS=1000,thresholdsSS=0.95,buildMethod="lasso")
{
  ptm <- proc.time()
  dashed.line <- "--------------------------------------------------\n"
  ...
  to.cat <- c(to.cat, "\n")
  Rsmlx:::print.result(print, summary.file, to.cat = to.cat, to.print = NULL)
  print.line <- F
  ...
  if (model$covariate) {
    ...
    Rsmlx:::print.result(print, summary.file, to.cat = to.cat, to.print = to.print)
  }
  if (model$correlation) {
    to.cat <- "\nCorrelation model:\n"
  ...
  if (iop.ll) {
    ...
    to.print <- round(ll,2)
    Rsmlx:::print.result(print, summary.file, to.cat=to.cat, to.print=to.print) 
    #    Rsmlx:::print.result(print, summary.file, to.cat=plain.line, to.print=NULL) 
  ...
  test.SAMBA2 = list(lastWrongIter=NULL,cumulativeWrong=0)
  while (!stop.test) {
    iter <- iter + 1
    ...
    if (!stop.test) {
      ...
      if(ll0[[criterion]]<ll[[criterion]]){
        if(is.null(test.SAMBA2$lastWrongIter)){
          test.SAMBA2 = list(lastWrongIter=iter,cumulativeWrong=1)
        }else if(test.SAMBA2$lastWrongIter==iter-1){
          test.SAMBA2 = list(lastWrongIter=iter,cumulativeWrong=test.SAMBA2$cumulativeWrong+1)
        }else{
          test.SAMBA2 = list(lastWrongIter=iter,cumulativeWrong=1)
        }
        print(test.SAMBA2)
        if(test.SAMBA2$cumulativeWrong>=3){
          stop.test=TRUE
        }
        to.cat <- paste0("\nNew model hasn't a better ",criterion," than the previous one.\nThe previous model is resset and reestimate. :\n")
        Rsmlx:::print.result(print, summary.file, to.cat=to.cat) 
        to.cat <- "\nCovariate model:\n"
        to.print <- Rsmlx:::formatCovariateModel(covariate.model0, cov.ini)
        Rsmlx:::print.result(print, summary.file, to.cat=to.cat, to.print=to.print) 
        
        Rsmlx:::mlx.setCovariateModel(covariate.model0)
      ...
      while (test.cor) {
        ...
        if (length(i.min)>0) {
          ...
          if (test.cor) {
            ...
            Rsmlx:::print.result(print, summary.file, to.cat=NULL, to.print=to.print) 
  ...
  if (iop.ll) {
    ll.final <- Rsmlx:::compute.criterion(criterion, method.ll, weight, pen.coef)
    ll <- Rsmlx:::formatLL(Rsmlx:::mlx.getEstimatedLogLikelihood()[[method.ll]], criterion, ll.final, is.weight, is.prior)
  }
