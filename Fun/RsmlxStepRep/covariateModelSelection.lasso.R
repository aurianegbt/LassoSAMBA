covariateModelSelection.lasso <- function(nfolds = 5,
  ...
  # Y et X
  # Y matrice des paramètres : ici on veut une matrice sans id et rep, qui a sur chaque colonne un parmaètre et sur chaque ligne un individu dont le paramètre est la moyenne des rep
  # X matrice des covariables : la matrice des covariables juste
  N = length(unique(Y$id))

  Y.matList = lapply(split(Y,Y$rep),function(x){x[,-c(1,2)]}) #1 : rep 2 : id
  X.mat  = covariates[,setdiff(colnames(covariates),"id")]


  r.var <- applyMethodLasso(Y.matList,X.mat,Sigma,alpha,cov0.list,
                            stabilitySelection,nfolds,nSS,thresholdsSS,
                            parallel,ncores,covariate.model=covariate.model,critMV=critMV)  # contient le résultat de seulement les paramètres variables donc pas dans l'ordre et pas entier
  r <- res <- r.cov0 <- list()
  for(j in 1:n.param){
    nj=param.names[j]
    if(indvar[j]){
