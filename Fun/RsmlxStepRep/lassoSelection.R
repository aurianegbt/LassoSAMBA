lassoSelection <- function(Y,X,Sigma=NULL,alpha=1,cov0.list=NULL, # cov0.list dans le même ordre que Y !!!!
                           ...
                           parallel=FALSE,ncores=1,printFrequencySS = FALSE){
  if(!is.matrix(Y)){
    Y <- as.matrix(Y)
  }
  if(!is.matrix(X)){
  ...
  if(parallel){doParallel::registerDoParallel(ncores)}
  ...
  if(!is.null(exclude) && ncol(Xwh)-length(exclude)==0){
    ...
    resCV = glmnet::cv.glmnet(Xwh, Ywh, alpha = alpha , parallel = parallel, nfolds=nfolds , exclude=exclude)
