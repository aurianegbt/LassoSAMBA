covariateModelSelection.lasso <- function(nfolds = 5,
  ...
  r.var <- lapply(param.names[which(indvar)],FUN=function(x){applyMethodLasso(Y.mat[,stringr::str_detect(colnames(Y.mat),x),drop=F],X.mat,Sigma[x,x],alpha,cov0.list[[x]],
