covariateModelSelection.rsharp <- function(nfolds = 5,
                                          alpha = 1,
                                          covFix = NULL,
                                          pen.coef=NULL,
                                          weight=1,
  ...
  Y.mat = setNames(lapply(
    lapply(names(indvar)[which(indvar)],FUN=function(x){Y[,c(1,which(stringr::str_detect(colnames(Y),x)))]}),
    FUN=function(y){lapply(split(y,f=y$rep),FUN=function(x){x[,-1,drop=F]})}),names(indvar)[which(indvar)])
  ...
  r.var <- setNames(lapply(names(indvar)[which(indvar)],FUN=function(p){
    Y.matp = Y.mat[[p]]
    cov0 = cov0.list[[p]]
    applyMethodrsharp(Y.matp,X.mat,Sigma[p,p],cov0,p.name=p,nfolds=nfolds,alpha=alpha)
