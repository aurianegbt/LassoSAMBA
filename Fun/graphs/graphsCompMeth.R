graphsCompMethod <- function(Folder,subtitle,project,covariateSize,buildMethod,JPEG,PNG){
  ...
  for(t in c("cov","corcov")){
    for(meth in buildMethod){
      ...
      valuemax = max(if(length(resultCovariateCov[resultCovariateCov$TypeOfSim==t & resultCovariateCov$Method == meth & !(resultCovariateCov$Covariate %in% unlist(H1.all,use.names = F)),"SelectedInDistinctModel"])==0){0}else{resultCovariateCov[resultCovariateCov$TypeOfSim==t & resultCovariateCov$Method == meth & !(resultCovariateCov$Covariate %in% unlist(H1.all,use.names = F)),"SelectedInDistinctModel"]}*100,na.rm = TRUE)
  ...
  cov = ggplot(resultCovariateCov[resultCovariateCov$TypeOfSim=="cov",],aes(x=factor(Covariate,levels=orderList$cov), y = SelectedInDistinctModel))+
    ...
    geom_text(data = resultModelCov[resultModelCov$TypeOfSim=="cov",],mapping=aes(label=text),x=covariateSize+0.5,y=1,hjust=1,size=7,vjust=1)+
    coord_cartesian(ylim=c(0,1),clip="off")

  df = valueDisplay[valueDisplay$type=="cov" & valueDisplay$cov=="cov",]

  for(i in 1:(nrow(df)/length(buildMethod))){
    eval(parse(text=paste0('cov <- cov + geom_text(data=df[seq(i,nrow(df),nrow(df)/length(buildMethod)),],mapping=aes(label=distext,x=length(Reduce(union,H1.all))+0.65),y=0.9-0.20*(i-1),color=fill.vec[i],size=7,hjust=0)')))
  }
  ...
  corcov = ggplot(resultCovariateCov[resultCovariateCov$TypeOfSim=="corcov",],aes(x=factor(Covariate,levels=orderList$corcov), y = SelectedInDistinctModel))+
    ...
    geom_text(data = resultModelCov[resultModelCov$TypeOfSim=="corcov",],mapping=aes(label=text),x=covariateSize+0.5,y=1,hjust=1,size=7,vjust=1)+
    coord_cartesian(ylim=c(0,1),clip="off")

  df = valueDisplay[valueDisplay$type=="corcov" & valueDisplay$cov=="cov",]

  for(i in 1:(nrow(df)/length(buildMethod))){
    eval(parse(text=paste0('corcov <- corcov + geom_text(data=df[seq(i,nrow(df),nrow(df)/length(buildMethod)),],mapping=aes(label=distext,x=length(Reduce(union,H1.all))+0.6),y=0.9-0.2*(i-1),color=fill.vec[i],size=7,hjust=0)')))
  }

  # Save plot
  annotate_figure(ggarrange(cov, corcov, nrow = 2),
                  top=text_grob(stringr::str_wrap(subtitle,100),size=15),
  ) %>%
    annotate_figure(
      top=text_grob("covariates presence in final model according to the method used",
                    face="italic",size=14,color="#9c9c9c")
    )%>%
    annotate_figure(
  ...
  if(PNG){
    ggsave(paste0(Folder,"/NumberOfSelection_",paste0(sapply(buildMethod,function(x){toupper(stringr::str_sub(x,end=2))}),collapse="-"),"_Cov.png"),
           height = length(buildMethod)/2*1500, width = list("10"=4200,"50"=5000,"200"=6000,"500"=7000)[[as.character(covariateSize)]], units = "px", bg='transparent',device=grDevices::png)
  }
  ...
  if(JPEG){
    ...
    ggsave(paste0(Folder,"/NumberOfSelection_",paste0(sapply(buildMethod,function(x){toupper(stringr::str_sub(x,end=2))}),collapse="-"),"_Cov.jpeg"),
           height = length(buildMethod)/2*1500, width = list("10"=4200,"50"=5000,"200"=6000,"500"=7000)[[as.character(covariateSize)]], units = "px",device=grDevices::jpeg)
  }
