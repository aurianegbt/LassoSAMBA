graphsParCompMethod <- function(Folder,subtitle,project,covariateSize,buildMethod,JPEG,PNG){
  ...
  cov = ggplot(CovariateModelSelectionCov[CovariateModelSelectionCov$TypeOfSim=="cov",],aes(x=factor(Covariate,levels=orderList$cov)))+
    ...
    geom_text(data=valueDisplay[valueDisplay$type=="cov" & valueDisplay$cov=="cov",],mapping=aes(label=distext,x=coor,y=if(project=="Pasin"){max(resultCovariateParCov[resultCovariateParCov$TypeOfSim=="cov","NumberofModel"])*0.9}else{max(resultCovariateParCov[resultCovariateParCov$TypeOfSim=="cov","NumberofModel"])*(0.80-(coor-2)*0.2)},hjust=0,vjust=1),color=rev(colDisplay),size=7)+
  ...
  corcov = ggplot(CovariateModelSelectionCov[CovariateModelSelectionCov$TypeOfSim=="corcov",],aes(x=factor(Covariate,levels=orderList$corcov)))+
    ...
    theme(axis.text.x = element_text(size = 10, angle = 90))+
    theme(axis.text.y = element_text(size = 10))+
    ylim(c(0,max(resultCovariateParCov[resultCovariateParCov$TypeOfSim=="corcov","NumberofModel"])))+
    ...
    theme(plot.title = element_text(size=25,color="#ee6c4d"))+
    theme(plot.subtitle = element_text(size=12))+
    geom_text(data=valueDisplay[valueDisplay$type=="corcov" & valueDisplay$cov=="cov",],mapping=aes(label=distext,x=coor,y=if(project=="Pasin"){max(resultCovariateParCov[resultCovariateParCov$TypeOfSim=="cov","NumberofModel"])*0.9}else{max(resultCovariateParCov[resultCovariateParCov$TypeOfSim=="cov","NumberofModel"])*(0.80-(coor-2)*0.2)},hjust=0,vjust=1),color=rev(colDisplay),size=7)+
    ...
    geom_text(data=valueModel[valueModel$TypeOfSim=="corcov",],x=lim$corcov+0.5,y=max(resultCovariateParCov[resultCovariateParCov$TypeOfSim=="corcov","NumberofModel"]),hjust=1,size=7,vjust=1,mapping=aes(label=text))+
    coord_cartesian(ylim=c(0,max(resultCovariateParCov[resultCovariateParCov$TypeOfSim=="corcov","NumberofModel"])),clip="off") 
