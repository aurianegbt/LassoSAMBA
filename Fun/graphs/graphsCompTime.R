...
# Cov graphs
  ...
  # Comparative graphs
  ggplot(computationStatsCov[computationStatsCov$TypeOfSim %in% c("cov","corcov"),],aes(color=Method,fill=Method,x=factor(Method,levels=c("reg","lassoSS","elasticnetSS","lassoSSCrit","elasticnetSSCrit",buildMethod[stringr::str_detect(buildMethod,"regPEN")],"lassoSSREP","elasticnetSSREP","lassoSSCritREP","elasticnetSSCritREP")),y=iteration))+
    geom_violin(lwd=0.5,alpha=0.6)+
    facet_grid(.~TypeOfSim,labeller = as_labeller(c(cov="With uncorrelated covariates",corcov="With correlated covariates")))+
  ...
  # Cov graphs
  ggplot(computationStatsCov[computationStatsCov$TypeOfSim == "cov",],aes(color=Method,fill=Method,x=factor(Method,levels=c("reg","lassoSS","elasticnetSS","lassoSSCrit","elasticnetSSCrit",buildMethod[stringr::str_detect(buildMethod,"regPEN")],"lassoSSREP","elasticnetSSREP","lassoSSCritREP","elasticnetSSCritREP")),y=iteration))+
    geom_violin(lwd=0.5,alpha=0.6)+
    geom_text(data=valueDisplayTime[valueDisplayTime$TypeOfSim=="cov",], mapping=aes(label=textMean,y=Mean,x=coor),vjust=-0.2,fontface="bold",size=6)+
    xlab("Method used")+
    ylab("Iteration Count")+
  ...
  # Corcov graphs
  ggplot(computationStatsCov[computationStatsCov$TypeOfSim == "corcov",],aes(color=Method,fill=Method,x=factor(Method,levels=c("reg","lassoSS","elasticnetSS","lassoSSCrit","elasticnetSSCrit",buildMethod[stringr::str_detect(buildMethod,"regPEN")],"lassoSSREP","elasticnetSSREP","lassoSSCritREP","elasticnetSSCritREP")),y=iteration))+
    geom_violin(lwd=0.5,alpha=0.6)+
    geom_text(data=valueDisplayTime[valueDisplayTime$TypeOfSim=="corcov",], mapping=aes(label=textMean,y=Mean,x=coor),vjust=-0.2,fontface="bold",size=6)+
    xlab("Method used")+
    ylab("Iteration Count")+
