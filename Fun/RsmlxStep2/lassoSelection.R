lassoSelection <- function(Y,X,Sigma=NULL,alpha=1,cov0.list=NULL, # cov0.list dans le même ordre que Y !!!!
                           ...
                           critMV="BIC",printFrequencySS = FALSE){
  if(!is.matrix(Y)){
    Y <- as.matrix(Y)
  }
  if(!is.matrix(X)){
  ...
  if(all(as.logical(selection))){
    eval(parse(text=(paste0("criterion = ",critMV,"(lm(Ywh ~ NULL))"))))
  }else{
    Xkeep = Xwh[,as.logical(selection)]
    eval(parse(text=(paste0("criterion = ",critMV,"(lm(Ywh~Xkeep))"))))
  }

  selection <- matrix(selection,nrow=ncol(Y),byrow=TRUE)
  colnames(selection) <- cov.names
  rownames(selection)<- colnames(Y)
  return(list(selection=selection,criterion=criterion))
}
